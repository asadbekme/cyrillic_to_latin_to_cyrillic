...
# Unicode'da 5 xil apostrof bor — hammasi bitta shaklga keltiriladi.
#   U+0027  '  straight (keyboard) apostrophe
#   U+02BC  ʼ  modifier letter apostrophe
#   U+02BB  ʻ  modifier letter turned comma
#   U+2018  '  left single quotation mark
#   U+2019  '  right single quotation mark
# ────────────────────────────────────────────────────────────────────
_APOSTROPHE_VARIANTS = ['\u0027', '\u02bc', '\u2019']   # straight, ʼ, right-curly

def _normalize_apostrophes(text: str) -> str:
    for variant in _APOSTROPHE_VARIANTS:
        text = text.replace(variant, '\u02bb')   # hammasi → ʻ
    text = text.replace('\u02bb', '\u2018')      # ʻ → ' (compounds bilan mos)
    return text
